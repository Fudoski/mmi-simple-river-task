...
import static com.bibik.university.mmi.context.StateConstant.BOAT;
import static com.bibik.university.mmi.context.StateConstant.CABBAGE;
import static com.bibik.university.mmi.context.StateConstant.ENTITIES_AMOUNT;
import static com.bibik.university.mmi.context.StateConstant.GOAT;
import static com.bibik.university.mmi.context.StateConstant.WOLF;
...
public class BasePathFinder implements PathFinder {
    ...
    public List<Path> find(State inputState) {
        ...
        Path path = new PathImpl( new PathElementImpl( inputState, null ) );
        queue.add( path );
            ...
            if (element != null && element.applyActionAndGetState().equals( goalState )) {
                ...
                for (State action : filterActions) {
                    ...
                    if (!visitedStates.contains( state ) && !failStates.contains( state )) {
                        ...
                        aNew.add( new PathElementImpl( current, action ) );
                        queue.add( aNew );
                    }
                }
            }
    ...
    private static List<State> filterFailStates(List<State> states) {
        return states.stream()
                .filter( state ->
                        WOLF_COAT_RULE.test( state.getAsArray() ) ||
                                GOAT_CABBAGE_RULE.test( state.getAsArray() )
                )
    ...
    private List<State> filterActions(State state, List<State> actions) {
        final byte boatSide = state.getAsArray()[BOAT];
    ...
    private static int createPositionMask(byte boatSide, State state) {
        ...
        byte[] stateArray = state.getAsArray();
